...
function dYdt = myODE_A0_A2_v0(t, Y)
    ...
    % Gene A parameters
    k_off_A = 0.5;   % Transition rate: OFF → ON
    k_on_A = 0.5;    % Transition rate: ON → OFF
    gamma_A = 1;     % mRNA degradation rate for gene A
    A1 = 0;          % Amplitude of oscillation for v1
    omega_A = 1;     % Frequency of oscillation
    v1 = 20 + A1 * cos(omega_A * t);  % Transcription rate for gene A
    
    % Gene B parameters
    k_off_B = 0.5;   % Transition rate: OFF → ON
    k_on_B = 0.5;    % Transition rate: ON → OFF
...
function dYdt = myODE_A5_A2_v0(t, Y)
    ...
    k_on_A = 0.5;    % Transition rate: ON → OFF
    gamma_A = 1;     % mRNA degradation rate for gene A
    A1 = 5;          % Amplitude of oscillation for v1
    omega_A = 1;     % Frequency of oscillation
    v1 = 20 + A1 * cos(omega_A * t);  % Transcription rate for gene A
...
function dYdt = myODE_A10_A2_v0(t, Y)
    ...
    k_on_A = 0.5;    % Transition rate: ON → OFF
    gamma_A = 1;     % mRNA degradation rate for gene A
    A1 = 10;         % Amplitude of oscillation for v1
    omega_A = 1;     % Frequency of oscillation
    v1 = 20 + A1 * cos(omega_A * t);  % Transcription rate for gene A
