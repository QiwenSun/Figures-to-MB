...
function dYdt = myODE_v2_zero(t, Y, omega)
    ...
    A1 = 5;          % Amplitude of periodic transcription for gene α
    V0_A = 20;       % Basal synthesis rate for gene A
    v1 = V0_A + A1 * cos(omega * t);
    
    % System parameters - Gene β (v2=0!)
    ...
    dp0dt = k_off_A * p1 - k_on_A * p0;      % p0 rate of change
    dp1dt = k_on_A * p0 - k_off_A * p1;      % p1 rate of change
    
    % Gene A mRNA dynamics
    dmAdt = -gamma_A * mA + v1 * p1;     % mA rate of change
    
    % Gene B promoter dynamics
    dq0dt = k_off_B * q1 - k_on_B * q0;      % q0 rate of change
    dq1dt = k_on_B * q0 - k_off_B * q1;      % q1 rate of change
    
    % Gene B mRNA dynamics (regulated by Gene A, and v2=0)
    dmBdt = -gamma_B * mB + (V0_B + A2 * mA) * q1;  % Key modification: v2=0
